



var dateControl = document.querySelector('input[type="date"]');
dateControl.value = 'YYYY-MM-DD';





{/* <form>
    <div>
        <label for="party">Choose your date (February 19th to March 21st):</label>
        <input type="date" id="start" name="bitcoin" value="YYYY-MM-DD" min="2019-02-19" max="2019-03-21" required>
            <span class="validity"></span>
  </div>
        <div>
            <input type="submit">
  </div>
</form> */}



//HTML for the date input:
{/* <label for="start">Start date:</label>

    <input type="date" id="start" name="trip-start"
        value="YYYY-MM-DD"
        min="2018-01-01" max="2018-12-31"> */}



const bitcoinRates = {
    "2019-02-19": 3922.0983,
    "2019-02-20": 3976.79,
    "2019-02-21": 3936.34,
    "2019-02-22": 3979.995,
    "2019-02-23": 4145.4383,
    "2019-02-24": 3767.305,
    "2019-02-25": 3852.16,
    "2019-02-26": 3830.6733,
    "2019-02-27": 3832.26,
    "2019-02-28": 3826.0233,
    "2019-03-01": 3850.0717,
    "2019-03-02": 3845.1133,
    "2019-03-03": 3820.1467,
    "2019-03-04": 3732.5733,
    "2019-03-05": 3880.8,
    "2019-03-06": 3884.9983,
    "2019-03-07": 3891.7317,
    "2019-03-08": 3875.47,
    "2019-03-09": 3950.61,
    "2019-03-10": 3932.0433,
    "2019-03-11": 3882.615,
    "2019-03-12": 3891.3467,
    "2019-03-13": 3882.8017,
    "2019-03-14": 3885.9883,
    "2019-03-15": 3935.415,
    "2019-03-16": 4022.5383,
    "2019-03-17": 3999.6617,
    "2019-03-18": 4004.8117,
    "2019-03-19": 4033.7183,
    "2019-03-20": 4064.0867,
    "2019-03-21": 3999.3233
},